...
public class GuiFishTank extends GuiMariculture {
	...
	protected void onMouseClick(int x, int y)  {
		if(mouseX >= -18 && mouseX <= 2 && mouseY >= 104 && mouseY <= 124) {
			...
			Minecraft.getMinecraft().thePlayer.sendQueue.addToSendQueue(
					new Packet116GUIClick(tile.xCoord, tile.yCoord, tile.zCoord, KeyHelper.getPlayer().entityId).build());
			KeyHelper.getPlayer().openGui(Mariculture.instance, -1, tile.worldObj, tile.xCoord, tile.yCoord, tile.zCoord);
		}
