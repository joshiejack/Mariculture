...
public abstract class TileTurbineBase extends TileStorageTank implements IUpgradable, IMachine, IRedstoneControlled, IEnergyHandler, IPowered, ISidedInventory, IFaceable {
    ...
    public void updateEntity() {
        ...
        machineTick++;
        if (!worldObj.isRemote) if (onTick(20)) {
            if (hasGUI) {
    ...
    public void animate() {
        ...
        } else if (worldObj.isRemote && Client.TURBINE_ANIM) if (isAnimating) {
            angle = angle + 0.1;
    ...
    protected EnergyStage computeEnergyStage() {
        if (speed <= 2) return EnergyStage.BLUE;
        else if (speed <= 4) return EnergyStage.GREEN;
        else if (speed <= 8) return EnergyStage.YELLOW;
        else if (speed <= 20) return EnergyStage.ORANGE;
    ...
    public boolean rotate() {
        for (int i = orientation.ordinal() + 1; i <= orientation.ordinal() + 6; ++i) {
            ...
            if (tile instanceof IEnergyHandler) if (((IEnergyHandler) tile).canConnectEnergy(facing)) {
                setFacing(facing);
