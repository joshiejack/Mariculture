...
public class WorldGenKelp extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
	    ...
	    if(rand.nextInt(Math.max(1, WorldGen.KELP_FOREST_START_CHANCE)) == 0) genForest = true;
	    if(genForest && rand.nextInt(Math.max(1, WorldGen.KELP_FOREST_END_CHANCE)) == 0) genForest = false;
    	...
    	for(int l = 0; l < (genForest? 128: rand.nextInt(129)); l++) {
    		...
    		if(genForest && rand.nextInt(WorldGen.KELP_FOREST_CHEST_CHANCE) == 0) generateChest(world, rand, i1, j1 + 1, k1);
    		else {
		...
		if(world.getBlock(x, y - 1, z) == Blocks.sand) {
			...
			if (chest != null) {
				WeightedRandomChestContent.generateChestContents(rand, ChestGenHooks.getItems(WorldPlus.OCEAN_CHEST, rand), chest, 
						rand.nextInt(WorldGen.KELP_FOREST_CHEST_MAX_ITEMS - WorldGen.KELP_FOREST_CHEST_MIN_ITEMS) + WorldGen.KELP_FOREST_CHEST_MIN_ITEMS);
			}
		}
	}
}
