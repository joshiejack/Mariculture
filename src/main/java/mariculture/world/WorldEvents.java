...
public class WorldEvents {
    ...
    public void onWorldGen(InitMapGenEvent event) {
        if (WorldGen.NO_MINESHAFTS) if (event.type == EventType.MINESHAFT) {
            try {
        ...
        if (WorldGen.WATER_CAVES) if (event.type == EventType.CAVE) {
            try {
        ...
        if (WorldGen.WATER_RAVINES) if (event.type == EventType.RAVINE) {
            try {
                if (Extra.HAS_BOP) {
