...
public class LiquifierHandler implements ISmelterHandler {
	...
	public void addRecipe(RecipeSmelter recipe) {
		if(recipe.input2 != null) {
			...
			recipes.put(OreDicHelper.convert(recipe.input2) + "|" + OreDicHelper.convert(recipe.input), recipe);
	...
	public RecipeSmelter getResult(ItemStack input, ItemStack input2, int temp) {
		...
		if(recipe != null) {
			...
			if(temp < recipe.temp && temp != -1) return null;
			if(recipe.input2 != null) {
				if(input2.stackSize < recipe.input2.stackSize) {
					return null;
				}
			}
