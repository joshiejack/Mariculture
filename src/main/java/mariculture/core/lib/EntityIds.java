package mariculture.core.lib;

public class EntityIds {
	//Adding IDs here so I can keep track of which I've used so far, who knows why i have random numbers :D
	public static final int BASS = 41;
	public static final int FISHING = 42;
	public static final int FAKE_SQUIRT = 43;
	public static final int FAKE_ITEM = 44;
	public static final int WALL_ART = 45;
	public static final int SPEED_BOAT = 187;
	public static final int HAMMERHEAD = 200;
}
