...
public class PlansMeta {
    ...
    public static int getType(ItemStack stack) {
        ...
        if (stack.hasTagCompound()) return stack.stackTagCompound.getInteger("type");
    ...
    public static int getPlanType(Block block, World world, int x, int y, int z) {
        if (block instanceof BlockFence) return PlansMeta.FENCE;
    ...
    public static boolean matches(NBTTagCompound nbt1, NBTTagCompound nbt2, boolean checkHardness) {
        ...
        if (checkHardness) if (resist1 != resist2 || hardness1 != hardness2) return false;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (blocks1[i] != blocks2[i]) return false;
            if (theBlockMetas1[i] != theBlockMetas2[i] || theSides1[i] != theSides2[i]) return false;
        }
    ...
    public static boolean isTheSame(World world, int x, int y, int z, ItemStack stack) {
        ...
        if (tile != null) {
            tile.writeData(block.stackTagCompound);
        } else return false;
