                    ...
                    if (nbttagcompound.hasKey("Lore")) {
                        ...
                        if (nbttaglist1.tagCount() > 0) {
                            ...
                            for (int j = 0; j < nbttaglist1.tagCount(); ++j) {
                                String color = j == 0 ? mariculture.lib.util.Text.ORANGE : mariculture.lib.util.Text.GREY;
                                String name = nbttaglist1.getStringTagAt(j);
                                if (name.equals("")) {
                                    if (!labeled) {
                                        labeled = true;
                                        list.add(mariculture.lib.util.Text.RED + MCTranslate.translate("blacklisted"));
                                    }
                                } else {
                                    list.add(color + name);
                                }
