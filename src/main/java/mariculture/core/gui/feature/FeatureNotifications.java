...
public class FeatureNotifications extends Feature {
        ...
        for (NotificationType note : notifications)
            if (tile.isNotificationVisible(note)) {
                if (mouseX >= -21 && mouseX <= 0 && mouseY >= 8 + 23 * pos && mouseY <= 8 + 23 * pos + 21) {
                    tooltip.add(mariculture.lib.util.Text.RED + StatCollector.translateToLocal("notification." + note.toString().toLowerCase().replaceAll("_", "\\.")));
                    for (int i = 0; i < 5; i++) {
    ...
    public static enum NotificationType {
        NO_ROD(0, 238, 0xF0E68C), NO_BAIT(18, 238, 0x8B6969), NO_WATER(36, 238, 0x00CED1), NOT_FISHABLE(54, 238, 0x00B2EE), NO_RF(72, 238, 0xC1CDCD), NO_PLAN(90, 238, 0x00A3D9), MISSING_SIDE(108, 238, 0x888888), NO_FOOD(126, 238, 0x2DB200), NO_MALE(144, 238, 0x7396FF), NO_FEMALE(162, 238, 0xFF99FF), BAD_ENV(180, 238, 0x111111), NO_EGG(198, 238, 0xBDBDAE);
