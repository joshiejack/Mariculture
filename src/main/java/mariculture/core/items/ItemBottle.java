...
public class ItemBottle extends ItemMCMeta {
    ...
    public ItemStack onEaten(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (!world.isRemote) if (player.shouldHeal()) {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean bool) {
        ...
        if (meta == BottleMeta.VOID) {
            list.add(mariculture.lib.util.Text.PURPLE + MCTranslate.translate("blackhole"));
            return;
        }

        if (meta == BottleMeta.EMPTY) {
            list.add(mariculture.lib.util.Text.YELLOW + MCTranslate.translate("doubleBottle"));
            return;
        }
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        if (stack.getItemDamage() != BottleMeta.VOID) return false;
        ...
        else if (y == 255 && block.getMaterial().isSolid()) return false;
