		...
		for (int k = 0; k < j; ++k) {
			...
			if (enchantment.canApplyAtEnchantingTable(stack) || ((item == Items.book) && enchantment.isAllowedOnBooks())) {
				for (int l = enchantment.getMinLevel(); l <= enchantment.getMaxLevel(); ++l) {
					if(enchantment instanceof EnchantmentJewelry && enchantability <= enchantment.getMaxEnchantability(l) || !(enchantment instanceof EnchantmentJewelry)) {
						if (enchantability >= enchantment.getMinEnchantability(l)) {
							if (hashmap == null) {
								hashmap = new HashMap();
							}
	
							if(orig > 30) {
								if(enchantment != Enchantment.protection && enchantment.getMaxLevel() != 1 && !(enchantment instanceof EnchantmentJewelry)) {
