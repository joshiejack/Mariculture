...
public class ItemTemperatureControl extends ItemStorage implements IItemUpgrade {
	...
	public Slot getSlot(InventoryStorage storage, int i) {
		switch(i) {
			case 0: return new SlotHeating(storage, i, 62, 26);
			case 1: return new SlotHeating(storage, i, 80, 26);
			case 2: return new SlotHeating(storage, i, 98, 26);
			case 3: return new SlotHeating(storage, i, 71, 44);
			case 4: return new SlotHeating(storage, i, 89, 44);
		}
	...
	public boolean isItemValid(ItemStack stack) {
		if(stack.getItem() instanceof IItemUpgrade && !(stack.getItem() instanceof ItemTemperatureControl)) {
	...
	public int getTemperature(ItemStack stack) {
		...
		for(ItemStack item: invent) {
			if(item != null) {
				if(item.getItem() instanceof IItemUpgrade) {
	...
	public static class SlotHeating extends Slot {
		...
		public boolean isItemValid(ItemStack stack) {
			if(stack.getItem() instanceof IItemUpgrade && !(stack.getItem() instanceof ItemTemperatureControl)) {
				return ((IItemUpgrade)stack.getItem()).getTemperature(stack) != 0;
			} else return false;
		}
	}
}
