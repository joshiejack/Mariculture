...
public class GenLayerBeach extends GenLayerShore {
    public GenLayerBeach(long seed, GenLayer parent) {
        super(seed, parent);
    }

    //Code mostly from GenLayerShore
    @Override
    ...
    private boolean isOcean(int k) {
        if (isBiomeOceanic(k) && k != Biome.getIdForBiome(Biomes.FROZEN_OCEAN)) return true;
        return k == Biome.getIdForBiome(Biomes.RIVER) && k == Biome.getIdForBiome(Biomes.SWAMPLAND);
    }


    //These methods are here because i'm too lazy to add an AT or use Reflection for them all.
