...
public class FishDNABase {
    ...
    public boolean hasEggData(ItemStack egg) {
        if (category == null) return egg.stackTagCompound.hasKey(getEggString());
        else {
    ...
    public Integer getDNA(ItemStack stack) {
        ...
        if (category == null) {
            if (!stack.stackTagCompound.hasKey(getHigherString()) && stack.stackTagCompound.hasKey("SpeciesID")) {
    ...
    public Integer getLowerDNA(ItemStack stack) {
        if (stack == null || !stack.hasTagCompound()) return 0;
        if (category == null) {
            if (!stack.stackTagCompound.hasKey(getLowerString()) && stack.stackTagCompound.hasKey("lowerSpeciesID")) {
                FishSpecies species = Fishing.fishHelper.getSpecies(stack);
    ...
    public int[] getDNAList(ItemStack stack) {
        if (category == null) return stack.stackTagCompound.getIntArray(getEggString());
        else {
            NBTTagCompound tag = stack.stackTagCompound.getCompoundTag(category);
            if (tag == null) {
                tag = new NBTTagCompound();
    ...
    public String[] getScannedDisplay(ItemStack stack) {
        return new String[] { StatCollector.translateToLocal("mariculture.string." + getName()), "" + getDNA(stack), "" + getLowerDNA(stack) };
    }
}
