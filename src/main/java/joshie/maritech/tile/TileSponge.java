...
public class TileSponge extends TileEnergyHandler {
    ...
    public boolean isBlockSponge(int x, int y, int z) {
        if (Modules.isActive(Modules.worldplus)) return worldObj.getBlock(x, y, z) == Blocks.sponge;
    ...
    public void updateCoords() {
        ...
        posYFound = false;
        for (int i = 0; i < 48; i++) {
            if (!posXFound) if (isBlockSponge(xCoord + i, yCoord, zCoord)) {
                posXFound = true;
                posX = xCoord + i;
            }

            if (!negXFound) if (isBlockSponge(xCoord - i, yCoord, zCoord)) {
                negXFound = true;
                negX = xCoord - i;
            }

            if (!posZFound) if (isBlockSponge(xCoord, yCoord, zCoord + i)) {
                posZFound = true;
                posZ = zCoord + i;
            }

            if (!negZFound) if (isBlockSponge(xCoord, yCoord, zCoord - i)) {
                negZFound = true;
                negZ = zCoord - i;
            }

            if (!posYFound) if (isBlockSponge(xCoord, yCoord + i, zCoord)) {
                posYFound = true;
                posY = yCoord + i;
            }
        }
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            ...
            if (tick % 200 == 0) if (posXFound && negXFound && posZFound && negZFound && posYFound) {
